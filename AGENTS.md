...
- `CIICCTE-server-frontend/` — React 19 + Vite 8 + Tailwind 4 + TS 6. Entrypoint `src/main.tsx:6` -> `src/App.tsx:8` (`BrowserRouter` -> `src/screens/DashboardScreen.tsx:6`). `tsconfig.json:1` solo project-references. Componentes en `src/components/` (`DashboardSidebarComponent`, `DashboardMainContentComponent`, `DashboardTelemetryComponent`, `DashboardLinuxUsersComponent`), pantallas en `src/screens/`.
...
- Estetica frontend: medidas multiplo de 8px, fondo `bg-white`, primarios `blue-700`, bordes/lineas `neutral-500` (Tailwind). Solo desktop, sin responsive. Componentes separados cargados en `DashboardScreen`.
